...
サーバー不要のブラウザベースUI（`index.html`）で、CLI版と同等の機能を利用できます。

### セットアップ
1. `index.html` をブラウザ（Chrome推奨）で開く
2. 「🔑 Gemini APIキーについて」をクリックして取得手順を確認

### APIキーの取得
1. [Google AI Studio](https://aistudio.google.com/app/apikey) を開く
2. 「APIキーを作成」→「新しいプロジェクトでAPIキーを作成」を選択
3. 表示されたキー（AIza...）をコピー
4. ツールの入力欄に貼り付け

### 使用手順
1. **料金プラン選択**:
   - **無料プラン**: 1日500ページまで無料、1ページずつ処理（遅い）
   - **有料プラン**: クレジットカード登録後、複数ページ同時処理（高速）
2. **PDF選択**: ファイルを選択し、必要に応じてページ範囲を指定
3. **実行**: 「OCR処理を開始する」をクリック
4. **結果確認**: ログで処理状況を確認、結果をコピー可能

### 特徴
- **サーバー不要**: ブラウザのみで完全に動作
- **クライアント処理**: APIキーはブラウザに入力され、ローカルに保存
- **自動レイアウト判定**: CLI版と同じく見開き分割・段組判定を自動実行
- **エスケープキー対応**: モーダルはEscキーで閉じられる
...
- **Web版のセキュリティ**: APIキーはブラウザのlocalStorage に保存されます。共有マシンでの使用時はご注意ください。
